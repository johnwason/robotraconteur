...
namespace detail
{
	...
	{
	  STACK_OF(X509)* p;
	  ~x509_stack_cleanup() { if (p) ::sk_X509_pop_free(p,X509_free); }
	};

	BIO* make_buffer_bio(const boost::asio::const_buffer& b)
	{
	  return ::BIO_new_mem_buf(
		  const_cast<void*>(boost::asio::buffer_cast<const void*>(b)),
		  static_cast<int>(boost::asio::buffer_size(b)));
	}

	

	void add_certificate_authority_x509(boost::shared_ptr<boost::asio::ssl::context> context, boost::asio::const_buffer& buf)
	{
		::ERR_clear_error();
	...
	OpenSSLAuthContext::OpenSSLAuthContext(const NodeID& nodeid)
	{
		this->nodeid=nodeid;
	...
	void OpenSSLAuthContext::LoadPKCS12FromFile(boost::string_ref fname)
	{
		...
		if (SSL_CTX_use_PrivateKey(server_context->native_handle(),pkey)!=1) error=true;
		if (SSL_CTX_use_certificate(server_context->native_handle(),cert)!=1) error=true;
		...
		if (!client_context)
		{
			...
			if (SSL_CTX_use_PrivateKey(client_context->native_handle(),(pkey))!=1) error=true;
			if (SSL_CTX_use_certificate(client_context->native_handle(),X509_dup(cert))!=1) error=true;
	...
	{
		boost::mutex::scoped_lock lock(mylock);
		if (!server_context) throw InvalidOperationException("Node certificate not loaded");
